...
@app.route('/producto/<int:id_Productos>')
def producto(id_Productos):
    conn = get_db_connection()
    cursor = conn.cursor()
    cursor.execute("SELECT Nombre_Producto, Precio_Producto, Descripcion_Producto, Url_imagen FROM Productos WHERE id_Productos = ?", (id_Productos,))
